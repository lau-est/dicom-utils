...
def extractDicomFileInfo(filename):
	"""
	Extract the tag information of Dicom files
	input :file name 
	...
	info["PatientBirthData"] = dcm.PatientBirthData # Patient's date of birth
	info["PatientAge"]       = dcm.PatientAge       # Patient age
	info['PatientSex']       = dcm.PatientSex       # Patient sex
	info['StudyID']          = dcm.StudyID          # Check ID
	info['StudyDate']        = dcm.StudyDate        # Check the date
	info['StudyTime']        = dcm.StudyTime        # Check the time
	info['InstitutionName']  = dcm.InstitutionName  # Institution name
	info['Manufacturer']     = dcm.Manufacturer     # Equipment manufacturer
...
def getDicomFile(dicom_dir, jpg_dir):
	...
	for f in all_file_list:
		...
		# dicom_info = extractDicomFileInfo(dicom_file_path)      # Extract tag information for backup
		jpg_name        = jpg_dir + f.split('.d')[0] + '.jpg'     # New jpg file name
